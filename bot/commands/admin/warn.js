...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try { await user.send({ embeds: [new EmbedBuilder().setColor(0xffa500).setTitle(`⚠️ تحذير من ${interaction.guild.name}`).setDescription(`**السبب:** ${reason}\n**إجمالي تحذيراتك:** ${count}`)] }); } catch {}
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xffa500).setTitle('⚠️ تم التحذير')
      .addFields({name:'👤 العضو',value:user.user.tag,inline:true},{name:'📝 السبب',value:reason,inline:true},{name:'🔢 الإجمالي',value:`${count} تحذير`,inline:true}).setTimestamp()] });
  }
};
